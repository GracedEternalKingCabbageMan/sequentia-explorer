...
lazy_static! {
    ...
    static ref TESTNET_INITIAL_ISSUANCE_PREVOUT: Txid =
        "0c52d2526a5c9f00e9fb74afd15dd3caaf17c823159a514f929ae25193a43a52"
            .parse()
            .unwrap();
    // SEQUENTIA chain=test: the genesis policy-asset issuance input references a
    // fabricated outpoint that is not a real UTXO, so it has no prevout to index.
    static ref SEQUENTIA_INITIAL_ISSUANCE_PREVOUT: Txid =
...
pub fn has_prevout(txin: &TxIn) -> bool {
    ...
    return !txin.is_coinbase()
        ...
        && txin.previous_output.txid != *SEQUENTIA_INITIAL_ISSUANCE_PREVOUT;
}
