...
pub enum Network {
    ...
    // SEQUENTIA: the Proof-of-Stake, Bitcoin-anchored testnet (chain=test).
    // Elements-based, so it lives under the liquid feature; decode the header
    // anchor by also enabling the elements/sequentia feature (electrs `sequentia`).
    #[cfg(feature = "liquid")]
    SequentiaTestnet,
}
...
impl Network {
    ...
    pub fn magic(self) -> u32 {
        match self {
            ...
            // SEQUENTIA chain=test pchMessageStart = ef 01 ba e0 (read LE from blk files).
    ...
    pub fn native_asset(self) -> &'static AssetId {
        match self {
            ...
            Network::SequentiaTestnet => &*asset::NATIVE_ASSET_ID_SEQUENTIA_TESTNET,
        }
    }
    ...
    pub fn pegged_asset(self) -> Option<&'static AssetId> {
        match self {
            ...
            Network::LiquidTestnet | Network::LiquidRegtest | Network::SequentiaTestnet => None,
        }
    }
...
impl From<&str> for Network {
    fn from(network_name: &str) -> Self {
        match network_name {
            ...
            #[cfg(feature = "liquid")]
            "sequentiatest" => Network::SequentiaTestnet,
